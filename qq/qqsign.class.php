...
class qqsign{
	...
	public function webqq($superkey){
		$supertoken=(string)$this->getToken($superkey);
		$url = "http://ptlogin2.qq.com/pt4_auth?daid=164&appid=501004106&pt4_shttps=1&auth_token=".$this->getToken($supertoken);
		...
		if(preg_match('/ptsigx=(.*?)&/',$data,$match)){
			...
			$url = 'http://s.web2.qq.com/api/get_self_info2?t=1465275659631';
			echo $data = $this->get_curl($url,0,'http://s.web2.qq.com/proxy.html',$cookie);exit;
		}else{
	...
	public function dongman(){
		$url = 'http://m.ac.qq.com/event/dailySignIn/SignIn.php?action=signIn&uin='.$this->uin.'&skey='.$this->skey.'&imei=&mac=&key=&t='.time().'1000';
		$data = $this->get_curl($url,0,$url);
		$arr = json_decode($data, true);
		if(array_key_exists('status',$arr) && $arr['status']==2){
			$this->msg[]='腾讯动漫签到成功！经验+'.$arr['reward']['exp'].',逗币+'.$arr['reward']['bonus'].',现有好感'.$arr['favor'];
		}elseif($arr['status']==1){
			$this->msg[]=$arr['msg'];
		}elseif($arr['code']==-99){
			$this->skeyzt=1;
			$this->msg[]='腾讯动漫签到失败！SKEY过期';
		}else{
			$this->msg[]=$arr['msg'];
		}
	}
	public function webface(){
		...
		if(@array_key_exists('result',$arr) && $arr['result']==0){
			...
			$data=$this->get_curl($url,$post,$url,$this->cookie);
			$this->msg[]='更换头像成功！更换头像的范围在你所使用过的头像中，修改头像种类请<a href="http://ptlogin2.qq.com/jump?uin='.$this->uin.'&skey='.$this->skey.'&u1=http%3A%2F%2Fstyle.qq.com%2Fface%2Fmanage.html" target="_blank">点此进入</a>';
